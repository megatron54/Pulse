...
- **Cuarto servicio añadido en sesión posterior: `scheduler` (PR #47).** Reutiliza el Dockerfile de `backend` (misma imagen, mismas dependencias), solo cambia el comando (`ensure_schema && exec python -m scheduler.app`). Antes de esto, el job nocturno de Garmin solo podía correr manualmente (`python -m scheduler.app` en un terminal dejado abierto) — con "arrancar todo con un solo comando" como objetivo explícito, la sincronización nocturna no podía depender de que alguien recordara dejar un terminal abierto.
- **Hallazgo real al containerizar el scheduler: el token OAuth de Garmin vivía en una ruta del host Windows** (`C:\Users\<usuario>\.garminconnect\pulse-user-<id>`, ver `scripts/garmin_pair.py`), invisible desde un contenedor Linux. Corregido con un volumen Docker nombrado (`garmin-tokens`, montado en `/data/garmin-tokens`) compartido entre `backend` y `scheduler`, y una variable de entorno nueva `PULSE_GARMIN_TOKENS_DIR` que `garmin_pair.py` respeta (con fallback a `~/.garminconnect` para seguir funcionando igual en local sin Docker). Migración manual de los datos reales del usuario (perfil, credenciales, métricas ya sincronizadas) desde el volumen de un proyecto compose anterior al nuevo, vía `pg_dump`/`psql`, sin pérdida de datos.
- **Hallazgo secundario:** el `HEALTHCHECK` heredado del Dockerfile de `backend` asume un servidor HTTP en `:8000/health` — el `scheduler` no expone ningún puerto (solo corre `BlockingScheduler` en primer plano), así que quedaba marcado `unhealthy` para siempre sin reflejar un problema real. Se sobrescribió con un healthcheck de liveness de proceso (`grep` sobre `/proc/*/cmdline`, ya que la imagen `python:3.12-slim` no trae `pgrep`/`procps`) en vez de deshabilitarlo del todo, para no perder toda señal de vida si el proceso muriera.
- Verificación end-to-end tras el cambio: sincronización real ejecutada dentro del propio contenedor `backend` (`SyncBatchResult(exitosos=1, fallidos=0, omitidos=0)`), con datos nuevos del día ya visibles en la base de datos (`sleep_score`, `hrv_value`, `body_battery_am` reales).
