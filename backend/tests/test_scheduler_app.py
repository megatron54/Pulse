...
class TestBuildScheduler:
    ...
    def test_feelfit_usa_hora_por_defecto_04_30_si_no_hay_env(self, monkeypatch):
        monkeypatch.delenv("PULSE_SCHEDULER_FEELFIT_HORA", raising=False)
        monkeypatch.delenv("PULSE_SCHEDULER_FEELFIT_MINUTO", raising=False)
        scheduler = build_scheduler()
        trigger = _job(scheduler, "sync_feelfit").trigger
        campos = {f.name: str(f) for f in trigger.fields}
        assert campos["hour"] == "4"
        assert campos["minute"] == "30"

    def test_feelfit_respeta_hora_configurada_por_env(self, monkeypatch):
        monkeypatch.setenv("PULSE_SCHEDULER_FEELFIT_HORA", "7")
        monkeypatch.setenv("PULSE_SCHEDULER_FEELFIT_MINUTO", "10")
        scheduler = build_scheduler()
        trigger = _job(scheduler, "sync_feelfit").trigger
        campos = {f.name: str(f) for f in trigger.fields}
